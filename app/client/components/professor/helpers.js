    ...
    (rating, index) =>
      rating.id <= 14
        ? {
            ...rating,
            average:
              Number(first[index].average) + Number(second[index].average)
          }
        : {
            ...rating,
            ...
            noAnswer:
              Number(first[index].noAnswer) + Number(second[index].noAnswer)
          }
  );
 ...
 */
const divideBy = numOfRatings => arr =>
  arr.map(
    rating =>
      rating.id <= 14
        ? {
...
const sortRatings = timesTaught => ratings => {
  ...
  return {
    ...
    clarity: normalized.filter(rating => clarity_q_ids.includes(rating.id)),
...
const getTimesTaught = (professors, courseId) =>
  professors.courses.reduce(
    (total, course) => (course.subject === courseId ? total + 1 : total),
    0
  );

const getAggregateRating = (ratings, times_taught) =>
  pipe(aggregateRatings, sortRatings(times_taught))(ratings);

const getRatingById = (ratings, ratingId, timesTaught) =>
  pipe(
    find(rating => rating._id === ratingId),
    prop("questions"),
    sortRatings(timesTaught)
  )(ratings);
